...
def model_wrapper(model, noise_schedule=None, is_cond_classifier=False, classifier_fn=None, classifier_scale=1., time_input_type='1', total_N=1000, model_kwargs={}):
    ...
    def model_fn(x, t_continuous):
        if is_cond_classifier:
            ...
            cond_emb2 = model_kwargs.get("cond_emb2", None)
            cond_mask2 = model_kwargs.get("cond_mask2", None)
            
            t_discrete = get_model_input_time(t_continuous)
            noise_uncond = model(x, t_discrete, cond_emb, cond_mask, cond_emb2, cond_mask2)
            if cond_emb2 is not None:
                cond_mask_c = torch.ones_like(cond_mask)
                cond_mask2_c = torch.ones_like(cond_mask2)
                cond_grad = model(x, t_discrete, cond_emb, cond_mask_c, cond_emb2, cond_mask2_c)
                
                scale1 = classifier_scale
                scale2 = classifier_scale*0.1
                return noise_uncond + \
                       scale1 * (cond_grad - noise_uncond) + \
                       scale2 * (cond_grad - noise_uncond)
            else:
                cond_mask_c = torch.ones_like(cond_mask)
            ...
            return model(x, t_discrete, 
                         model_kwargs['cond_emb'], model_kwargs['cond_mask'],
                         cond_emb2, cond_mask2)
